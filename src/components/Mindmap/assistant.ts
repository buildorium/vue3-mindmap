...
export const makeTransition = (
  ...
  return d3.transition<Mdata>().duration(dura).ease(easingFn) as d3.Transition<
    any,
    Mdata,
    null,
    undefined
...
export function selectGNode(d: SVGGElement | Mdata): void {
  const ele =
    d instanceof SVGGElement
      ? d
      : document.querySelector<SVGGElement>(`g[data-id='${getDataId(d)}']`);
...
export const moveNode = (
  ...
  d.px = p[0];
  d.py = p[1];
  d3.select<SVGGElement, Mdata>(node)
    .transition(tran)
...
export const fitView = (): void => {
  ...
  const multiple = Math.min(
    svgBCR.width / gBB.width,
    svgBCR.height / gBB.height
  );
...
export const bindForeignDiv = (): void => {
  if (foreignDivEle.value) {
    ...
    foreignDivEle.value.addEventListener("mousedown", (e: MouseEvent) =>
      e.stopPropagation()
    );
  }
};
